<div class="row positionrelative">
    <div class="col-md-10">
        <h5 class="modal-title newmodaltittle epic-tittle-header mr-4 pb-5" id="create-epic">
            <img class="objectiveheaderimage" src="{{ url('public/assets/svg/epicheaderheader.svg') }}">@if($data->epic_name) {{ $data->epic_name }} @else Enter Epic Tittle @endif
        </h5>
        <div class="d-flex">
                                    ...
                                    <div class="col-md-12 memberprofile memberprofilecontroleight" onclick="selectteamforepic({{$r->id}} , {{$data->id}})">
                                        <div class="row">
                                            ...
                                            <div class="col-md-8">
                                                <div class="membername">{{ $r->team_title }}</div>
                                                @if($r->lead_id)
                                                @if(DB::table('members')->where('id' , $r->lead_id)->first())
                                                <div class="memberdetail">Lead: {{ DB::table('members')->where('id' , $r->lead_id)->first()->name }} {{ DB::table('members')->where('id' , $r->lead_id)->first()->last_name }}</div>
                                                @endif
                                                @endif
                                            </div>
            ...
            @endif
            <a href="javascript:void(0)" onclick="rasiseflag({{$data->id}})" class="epic-header-buttons epicheaderteambutton" id="showboardbutton">
