        ...
        "Home": {
            "data": {
                "1": {
                    ...
                    "html": '<div class="row"><div class="col-md-4"> <div class="buisnessunit"> <div class="mainheading row mb-3"> <div class="col-md-12"> <h4 class="d-flex"><span class="material-symbols-outlined mr-2">domain</span> <span> {{$data->business_name}} {{$data->id}} </span></h4> </div> </div> @foreach(DB::table('objectives')->where('type' , 'unit')->where('unit_id'  ,$data->id)->get() as $o) <div class="row"> <div class="col-md-1"> <img src="{{ url('public/assets/svg/linkingbuisnessunit.svg') }}"> </div> <div class="col-md-8"> <div class="buisnessunit-card-subtittle"> <p class="buisnessunitheading">{{ $o->objective_name }}</p> </div> </div> <div class="col-md-3 text-right"> <div class="badge bg-success buisnessunitbadge"> {{ $o->obj_prog }}% </div> </div> <div class="col-md-12"> @foreach(DB::table('key_result')->where('obj_id' , $o->id)->get() as $key_result) <div class="row mt-2"> <div class="col-md-1"> <img src="{{ url('public/assets/svg/linkingkey.svg') }}"> </div> <div class="col-md-7"> <p class="buisnessunitlinkingtext">{{$key_result->key_name}}</p> </div> <div class="col-md-1"> @if(DB::table('team_link_child')->where('bussiness_key_id' , $key_result->id)->count() > 0) <img src="{{ url('public/assets/svg/link.svg') }}"> @endif </div> <div class="col-md-3 text-right"> <div class="badge buisnessunitbadge">{{$key_result->key_prog}}%</div> </div> </div> @endforeach </div> </div> @endforeach </div> </div></div>',
                    "typenode": false,
                    "inputs": {},
                    {!! DB::table('flow_chart_scripts')->where('unit_id' , $data->id)->where('type' , 'unit')->where('script_type' , 'output')->first()->script !!}
                    "pos_x": 10,
                    "pos_y": 20
                },
                ...
                "{{ 100+$v->id }}": {
                    ...
                    "html": '<div class="col-md-4"> <div class="buisnessunit"> <div class="mainheading row mb-3"> <div class="col-md-12"> <h4 class="d-flex"><span class="material-symbols-outlined mr-2">layers</span> <span> {{$v->value_name}} {{$v->id}}</span></h4> </div> </div> @foreach(DB::table('objectives')->where('type' , 'stream')->where('unit_id'  ,$v->id)->get() as $v_o) <div class="row"> <div class="col-md-1"> <img src="{{ url("public/assets/svg/linkingbuisnessunit.svg") }}"> </div> <div class="col-md-8"> <div class="buisnessunit-card-subtittle"> <p class="buisnessunitheading">{{ $v_o->objective_name }}</p> </div> </div> <div class="col-md-3 text-right"> <div class="badge bg-success buisnessunitbadge"> {{ $v_o->obj_prog }}% </div> </div> <div class="col-md-12"> @foreach(DB::table('key_result')->where('obj_id' , $v_o->id)->get() as $v_o_key_result) <div class="row mt-2"> <div class="col-md-1"> <img src="{{ url("public/assets/svg/linkingkey.svg") }}"> </div> <div class="col-md-7"> <p class="buisnessunitlinkingtext">{{$v_o_key_result->key_name}}</p> </div> <div class="col-md-1"> <img src="{{ url("public/assets/svg/link.svg") }}"> </div> <div class="col-md-3 text-right"> <div class="badge buisnessunitbadge">{{$v_o_key_result->key_prog}}%</div> </div> </div> @endforeach </div> </div> @endforeach </div> </div>',
                    "typenode": false,
                    "inputs": {
                        "input_1": {
                            "connections": [{
