...
@if (!isset($noreport))
    ...
    @if ($report && $KEYChart)
                    ...
                    <div class="displayflex">
                        @if ($KEYChart->cust_type == 'Custom')
                            <span class="mt-2" style="font-size: 13px">Frequency: {{ $KEYChart->repeatdays }} On
                                @if ($KEYChart->daysInput)
                                    @foreach ($days as $day)
                                        {{ $day }}
                                    @endforeach
                                @endif </span><a href="javascript:void(0)"
                                onclick="uploadfrequency()" class="nav-link">(Change)</a>
                        @elseif($KEYChart->cust_type == 'Does not repeat')
                            <span class="mt-2" style="font-size: 13px">Frequency: {{ $KEYChart->repeatdays }}
                            </span><a href="javascript:void(0)" onclick="uploadfrequency()"
                                class="nav-link">(Change)</a>
        ...
        <div class="row uploadfrequency displaynone mb-1">
            <div class="col-md-12">
                <div class="card">
                    <div class="card-body">
                        <form class="needs-validation savefrequencyform" action="{{ url('frequency-update') }}"
                            method="POST">
                            ...
                            <div class="d-flex flex-column mt-2">
                                <div class="mb-4 Custom"
                                    @if ($KEYChart->cust_type == 'Custom') @else style="display:none" @endif>
                                ...
                                <div class="d-flex flex-row align-items-center mb-4">
                                    <div class="mr-2 Custom"
                                        @if ($KEYChart->cust_type == 'Custom') @else style="display:none" @endif>
                                        Repeat every
                                    </div>
                                    <div class="mr-2 Custom"
                                        @if ($KEYChart->cust_type == 'Custom') @else style="display:none" @endif>
                                        <input class="form-control input-sm" value="{{ $KEYChart->days }}"
                                            type="number" min="1" name="days">
                                    </div>
                                    <div class="mr-2 Custom"
                                        @if ($KEYChart->cust_type == 'Custom') @else style="display:none" @endif>
                                        <select class="form-control input-sm" value="{{ $KEYChart->repeatdays }}"
                                            name="repeat">
                                ...
                                <div class="d-flex flex-row align-items-center mb-4">
                                    ...
                                    <div class="mr-2 Custom"
                                        ...
                                        @if ($KEYChart->cust_type == 'Custom')
                                            <label
                                                class="day-circle @if ($KEYChart->daysInput) {{ in_array('Sunday', $days) ? 'checked' : '' }}" @endif data-day="Sunday"
                                                onclick="toggleDay(this)">S</label>
                                            <label
                                                class="day-circle @if ($KEYChart->daysInput) {{ in_array('Monday', $days) ? 'checked' : '' }}" @endif data-day="Monday"
                                                onclick="toggleDay(this)">M</label>
                                            <label
                                                class="day-circle @if ($KEYChart->daysInput) {{ in_array('Tuesday', $days) ? 'checked' : '' }}" @endif data-day="Tuesday"
                                                onclick="toggleDay(this)">T</label>
                                            <label
                                                class="day-circle @if ($KEYChart->daysInput) {{ in_array('Wednesday', $days) ? 'checked' : '' }}" @endif data-day="Wednesday"
                                                onclick="toggleDay(this)">W</label>
                                            <label
                                                class="day-circle @if ($KEYChart->daysInput) {{ in_array('Thursday', $days) ? 'checked' : '' }}" @endif data-day="Thursday"
                                                onclick="toggleDay(this)">T</label>
                                            <label
                                                class="day-circle @if ($KEYChart->daysInput) {{ in_array('Friday', $days) ? 'checked' : '' }}" @endif data-day="Friday"
                                                onclick="toggleDay(this)">F</label>
                                            <label
                                                class="day-circle @if ($KEYChart->daysInput) {{ in_array('Saturday', $days) ? 'checked' : '' }}" @endif data-day="Saturday"
                                                onclick="toggleDay(this)">S</label>
                                        @else
                                            <label class="day-circle" data-day="Sunday"
                                                onclick="toggleDay(this)">S</label>
                                            <label class="day-circle" data-day="Monday"
                                                onclick="toggleDay(this)">M</label>
                                            <label class="day-circle" data-day="Tuesday"
                                                onclick="toggleDay(this)">T</label>
                                            <label class="day-circle" data-day="Wednesday"
                                                onclick="toggleDay(this)">W</label>
                                            <label class="day-circle" data-day="Thursday"
                                                onclick="toggleDay(this)">T</label>
                                            <label class="day-circle" data-day="Friday"
                                                onclick="toggleDay(this)">F</label>
                                            <label class="day-circle" data-day="Saturday"
                                                onclick="toggleDay(this)">S</label>
                                        @endif
                                    </div>

                                </div>
                                <input type="hidden" id="checkedDays"
                                    @if ($KEYChart->daysInput) value="{{ $KEYChart->daysInput }}" @endif
                                    name="daysInput[]" value="">

                                <button type="submit"
                                    class="saveepicflagbuttona btn btn-primary btn-sm">Save</button>
                        </form>
                    </div>
                ...
                <div>
                    <div class="dropdown d-flex align-items-center">
                        ...
                        <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                            ...
                            <a class="dropdown-item" data-toggle="modal"
                                onclick="deletequartervalue({{ $val->id }})" data-target="#delete22">Delete</a>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <div class="col-md-12 col-lg-12 col-xl-12 displaynone writecomment{{ $val->id }}">
        <div class="d-flex flex-column">
            <form method="POST" id="savecommentkey{{ $val->id }}"
                action="{{ url('keyresult-savecomment') }}">
                @csrf
                <input type="hidden" value="{{ $val->id }}" name="flag_id">
    ...
    $(function() {
        $("#datepicker").datepicker({
            onSelect: function(dateText, inst) {
                ...
                $('#datepickerselect').html(
                    '<option value="Does not repeat">Does not repeat</option><option value="Daily">Daily</option><option value="Weekly">Weekly on ' +
                    fullDayName + '</option><option value="Custom">Custom...</option>');
